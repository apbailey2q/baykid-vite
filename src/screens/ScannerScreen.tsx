...
import { demoScanRewardSplit, activeFundraiser } from '../lib/demoFundraisers'
...
function FakeQR() {
  return (
    <svg width="96" height="96" viewBox="0 0 24 24" opacity={0.82}>
      {/* Top-left finder */}
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            style={{
              ...
              border:     soundOn ? '1px solid rgba(0,200,255,0.28)' : '1px solid rgba(255,255,255,0.1)',
              cursor:     'pointer',
            }}
          >
            <span style={{ fontSize: 11, lineHeight: 1 }}>{soundOn ? '🔊' : '🔇'}</span>
            <span className="text-[10px] font-semibold" style={{ color: soundOn ? '#00c8ff' : 'rgba(255,255,255,0.3)' }}>
              {soundOn ? 'On' : 'Off'}
            </span>
          </button>
          <Link
      ...
      <div className="relative flex-1 overflow-y-auto pb-24" style={{ zIndex: 1 }}>
        <div className="max-w-[440px] mx-auto px-4 pt-8 pb-8">

          {/* ── Camera Mode label ─────────────────────────────────────────────── */}
          <div className="flex justify-center mb-4" style={fade(0)}>
            <div
          ...
          {/* ── Camera frame ──────────────────────────────────────────────────── */}
          <div className="flex justify-center mb-3" style={fade(40)}>
            <div
            ...
            >
              {/* Camera grid overlay */}
              <svg className="absolute inset-0 pointer-events-none" width="100%" height="100%" style={{ opacity: 0.09 }}>
              ...
              {/* Corner brackets */}
              {BRACKETS.map((s, i) => (
                <div key={i} className="absolute" style={{ ...s, width: 28, height: 28, transition: 'border-color 0.4s ease' }} />
              ))}
              ...
              {/* Fake QR — dims while scanning, hidden when done */}
              {!isDone && (
                <div
              ...
              {isScan && (
                ...
                >
                  <span className="w-3 h-3 rounded-full border-2 animate-spin" style={{ borderColor: 'rgba(0,200,255,0.25)', borderTopColor: '#00c8ff' }} />
                  <span className="text-[11px] font-semibold" style={{ color: '#00c8ff' }}>Scanning QR code…</span>
                </div>
              )}
          ...
          {/* Bottom camera label */}
          <p className="text-center text-[11px] mb-5" style={fade(60)}>
            {isDone
          ...
          {/* ── Scan Accuracy card (pre-scan only) ───────────────────────────── */}
          {!isDone && (
            <div
              className="rounded-2xl p-4 mb-4"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', ...fade(80) }}
            >
                ...
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: item.color, boxShadow: `0 0 5px ${item.color}` }} />
          ...
          {/* ── Fraud Detection link (pre-scan only) ─────────────────────────── */}
          {!isDone && (
            <div className="mb-4" style={fade(100)}>
              <Link
                to="/fraud-detection"
                className="w-full flex items-center gap-3 p-3.5 rounded-2xl transition-all hover:brightness-110"
                style={{ background: 'rgba(248,113,113,0.06)', border: '1px solid rgba(248,113,113,0.2)', textDecoration: 'none' }}
              >
                <div
                  className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
                  style={{ background: 'rgba(248,113,113,0.12)', border: '1px solid rgba(248,113,113,0.28)', fontSize: 15 }}
                >
                  🛡️
                </div>
                <div className="flex-1 min-w-0">
                  <p style={{ fontSize: 12, fontWeight: 700, color: '#ffffff' }}>Fraud Detection</p>
                  <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.38)', marginTop: 1 }}>View duplicate scan prevention &amp; payout protection</p>
                </div>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(248,113,113,0.5)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          {isDone && (
            <div style={{ animation: 'slideUp 0.4s ease' }}>
              ...
              {/* Result card */}
              <div className="rounded-2xl overflow-hidden mb-4" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(0,190,255,0.18)' }}>
                <div className="flex items-center gap-3 px-5 py-4" style={{ background: 'rgba(0,87,231,0.12)', borderBottom: '1px solid rgba(0,190,255,0.1)' }}>
                  <span style={{ fontSize: 22 }}>📦</span>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest mb-0.5" style={{ color: 'rgba(0,200,255,0.7)' }}>Scan Result</p>
                    <p className="text-sm font-bold" style={{ color: '#ffffff' }}>QR Bag Verified</p>
                  </div>
                </div>
                {resultRows.map((row) => (
                  <div key={row.label} className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{row.label}</span>
                    <span
                      className={row.mono ? 'font-mono' : ''}
                      style={{ fontSize: row.mono ? 11 : 12, fontWeight: 600, color: row.accent ? '#00c8ff' : '#ffffff', maxWidth: '55%', textAlign: 'right' }}
                    >
                      {row.value}
                    </span>
                  </div>
              ...
              <div className="flex gap-2 mb-5">
                {[
                  ...
                  <div key={chip.label} className="flex-1 flex flex-col items-center gap-1 rounded-xl py-2.5" style={{ background: 'rgba(0,200,128,0.07)', border: '1px solid rgba(0,200,128,0.2)' }}>
                    <span style={{ fontSize: 16 }}>{chip.icon}</span>
                    <span style={{ fontSize: 10, color: '#5eead4', fontWeight: 600, textAlign: 'center', lineHeight: 1.3 }}>{chip.label}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => navigate('/bag-inspection')}
                  className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl font-semibold text-sm"
                  style={{ background: 'linear-gradient(135deg, #0057e7, #00c8ff)', color: '#ffffff', border: 'none', cursor: 'pointer' }}
                >
      ...
      {showCelebration && (
        <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 50, pointerEvents: 'none' }}>
          {[
            ...
            <div
              ...
              style={{
                ...
                top: '42%', left: '50%', marginLeft: item.x,
          ...
          >
            <div style={{ background: 'rgba(0,0,0,0.72)', border: '1px solid rgba(0,200,128,0.5)', borderRadius: 16, padding: '10px 22px', boxShadow: '0 0 36px rgba(0,200,128,0.35)' }}>
              <span style={{ fontSize: 20, fontWeight: 700, background: 'linear-gradient(90deg, #00c8ff, #5eead4)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' } as React.CSSProperties}>
                Impact Created!
              </span>
            </div>
          </div>
