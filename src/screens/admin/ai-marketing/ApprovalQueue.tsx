...
export function ApprovalQueue() {
  if (WORKFLOW_V2) return <ApprovalQueueV2 />
...
function ApprovalQueueV2() {
  ...
  const v2StatusFilterOpts: Array<{ value: PostStatus | 'all'; label: string }> = [
    { value: 'all',              label: 'All'      },
    { value: 'pending_approval', label: STATUS_META_V2.pending_approval.label },
    { value: 'draft',            label: STATUS_META_V2.draft.label            },
    { value: 'rejected',         label: STATUS_META_V2.rejected.label         },
  ]
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      {grouped.length > 0 ? (
        grouped.map(({ status, posts: gPosts }) => {
          const meta = STATUS_META_V2[status]
          return (
            <div key={status} style={{ marginBottom: 32 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {gPosts.map((post) => {
                  ...
                  const isActionable = !isEditing && !isScheduling
                  const postMeta     = STATUS_META_V2[post.status]
                  return (
                    <div key={post.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      <div style={{ paddingTop: 20, flexShrink: 0 }}>
                        <input
