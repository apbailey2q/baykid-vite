  ...
  useEffect(() => {
    ...
    async function fetchData(isRefresh = false) {
      ...
      const [
        ...
        supabase.from('bag_scans').select('id, scan_time, location, bags(bag_code)').order('scan_time', { ascending: false }).limit(5),
        supabase.from('inspections').select('id, status, created_at, bags(bag_code)').order('created_at', { ascending: false }).limit(5),
