...
-- FK with ON DELETE CASCADE, created_at/updated_at + handle_updated_at trigger,
-- idempotent DO-block policies, NOTIFY pgrst at the end.
--
-- Depends on:
--   • 20260516_commercial_rls_complete.sql  → public.is_admin()
--   • 20260527_ai_marketing_schema.sql      → ai_orgs, ai_is_org_member(),
--                                             handle_updated_at()
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'billing_connected_accounts' AND policyname = 'billing_accts_member_write') THEN
    EXECUTE 'CREATE POLICY billing_accts_member_write ON public.billing_connected_accounts FOR INSERT TO authenticated WITH CHECK (true AND created_by = auth.uid())';
  END IF;
